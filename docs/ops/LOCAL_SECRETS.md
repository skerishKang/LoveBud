...
## 테스트 계정 운영 원칙 (2026-04-18)

### 파일 구조

| 파일 | 용도 | Git | 비고 |
|------|------|-----|------|
| `.local/test-accounts.json` | 실제 테스트 계정 | ❌ 무시 | 로컬에서만 관리 |
| `.local/test-accounts.example.json` | 예시 형식 | ✅ 포함 | 저장소 참조용 |
| `docs/ops/QA_CREDENTIALS.txt` | 계정 위치 안내 | ✅ 포함 | 경로/사용법만 기록 |

### 운영 체크리스트

- [ ] 저장소에 실제 비밀번호가 없는지 확인
- [ ] 문서에는 경로와 사용법만 기록
- [ ] `.gitignore`에 `.local/`이 포함되어 있는지 확인
- [ ] 새 계정 추가 시 `.local/test-accounts.json`에만 기록

### 보안 규칙

1. **절대 금지**: GitHub/저장소에 비밀번호 커밋
2. **문서 한계**: QA_CREDENTIALS.txt에도 실제 값 쓰지 않음
3. **로컬 전용**: 모든 실제 계정은 `.local/`에서 관리
4. **예시 분리**: 예시는 `.example.json`, 실제는 `.json`