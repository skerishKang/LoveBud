...
A separate read-only PostgreSQL catalog adapter can construct sanitized metadata from `pg_catalog` for **explicitly allowlisted synthetic objects** only:
...
Hard boundaries for this adapter:
