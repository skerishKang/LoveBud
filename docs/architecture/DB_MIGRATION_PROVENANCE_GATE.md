...
The PostgreSQL pinned-session advisory-lock adapter that implements the orchestrator's `acquireAdvisoryLock`/`checkAdvisoryLock`/`releaseAdvisoryLock` dependency contract — one global per-database lock keyed by a fixed two-int pair, a single pinned session from acquire through release, non-blocking `pg_try_advisory_lock`, current-session `pg_locks` verification, explicit `pg_advisory_unlock`, exactly-once pool release, an opaque frozen handle, and sanitized status-only results — is defined and source-tested in `docs/architecture/db-postgres-session-lock-adapter-contract.md`. It uses synthetic injected sessions/queries and performs no database connection, `pg` import, real lock, or actual query execution.

The PostgreSQL migration-ledger read/append adapter that implements the orchestrator's `readLedger`/`appendLedgerRecord` dependency contract — a fixed `schema_migration_ledger` relation and fixed seven-field order, an exact named read query (`lovebud-migration-ledger-read-v1`) and an append-only named query (`lovebud-migration-ledger-append-v1`, `ON CONFLICT (migration_id) DO NOTHING`), a fixed sanitized read rejection (`POSTGRES_LEDGER_READ_UNAVAILABLE`, never an empty array on failure), an `APPENDED|FAILED|UNKNOWN` append mapping, exact row evidence, an opaque forwarded `lockHandle`, and sanitized status-only results — is defined and source-tested in `docs/architecture/db-postgres-ledger-adapter-contract.md`. It uses synthetic injected queries and performs no database connection, `pg` import, real ledger read, or real ledger write.
