# PRODUCT_IDENTITY

> **"Digital Scrapbook of Emotions"**
> 처음 사랑에 빠진 순간부터 깊은 팬이 되어가는 모든 경로를 영상과 메모로 연결해 기록하고, 이를 아날로그 스크랩북처럼 감상하고 공유하는 팬 감정 러브트리.

---

## 1. Product Summary
Lovetree는 단순한 데이터 저장소나 북마크 서비스가 아닙니다. 팬들이 자신이 좋아하는 대상(아티스트, 콘텐츠 등)과 맺어가는 **정서적 관계의 궤적**을 시각화하는 도구입니다. 유튜브 영상의 특정 시점(Timestamp)에 담긴 벅찬 감정을 메모로 남기고, 이 순간들을 유기적으로 연결하여 하나의 '나무(Tree)'로 성장시키는 경험을 제공합니다.

## 2. Core Problem
* **Fragmented Memories**: 단편적인 영상 링크나 트윗으로는 그때의 감정이 왜 특별했는지, 어떤 맥락에서 이어졌는지 기억하기 어렵습니다.
* **Cold Interfaces**: 기존의 IT 도구들은 효율성만 강조하여, 팬의 소중한 추억을 담기에는 너무 차갑고 딱딱합니다.
* **Missing Paths**: 내가 어떻게 이 대상의 팬이 되었는지에 대한 '경로'를 한눈에 볼 수 있는 방법이 없습니다.

## 3. Core User (핵심 사용자)
Lovetree는 아래 조건에 해당하는 팬을 위해 설계되었습니다:

- **입덕 직후의 열정**: 특정 영상/시점의 감동으로 인해 덕질에 빠진 직후의 상태
- **경로 기록의 욕구**: 단순히 좋아하는 것이 아니라, "어떻게 팬이 되었는지"의物語를 남기고 싶어함
- **공유를 통한 연결**: 나만의 기록을 다른 팬과 공유하여 comunidade에 기여하고 싶어함
- **감정 중심의 소비**: 데이터 저장이나 정보 정리보다 감정 자체를 기록하는 것을 선호함

> Lovetree는 **"덕질의 첫 순간"**과 **"그 순간이 쌓여 만들어지는 경로"**에 가치를 둡니다.

## 4. Product Pillars (핵심 원칙)
1. **First Moment First**: "입덕"의 그 순간, 가장 강렬했던 첫 기억을 가장 소중하게 다룹니다.
2. **Connected Love Path**: 순간들을 파편화하지 않고, 나뭇가지처럼 유기적으로 흐르는 경로(Path)로 연결합니다.
3. **Emotion Over Archive**: 단순한 저장보다는 '왜 이 시점이 좋았는지'에 대한 감정 메모를 핵심 콘텐츠로 처리합니다.
4. **Private First, Share Second**: 나의 소중한 공간으로서의 가치를 먼저 확립한 뒤, 선택적으로 공유하여 팬덤 내에서 긍정적인 영향력을 전파합니다.

## 5. UX Direction
* **Warm & Organic**: 나뭇가지 같은 곡선, 부드러운 그림자, 따뜻한 색감을 사용하여 사용자의 감정을 보호합니다.
* **Tactile Experience**: 디지털 공간이지만 아날로그 다이어리를 꾸미는 듯한 종이 질감과 여백의 미를 추구합니다.
* **Emotional Flow**: 기술적인 플로우차트 스타일을 배격하고, 감정의 흐름을 닮은 자연스러운 인터페이스를 제공합니다.

## 6. Mobile vs Desktop Role
| 환경 | 주요 역할 | 사용자 경험 (UX) |
| :--- | :--- | :--- |
| **Mobile** | **빠른 기록 & 감상** | 이동 중에도 영상의 특정 시점과 짧은 감정 메모를 즉시 남기는 'Snap' 경험. |
| **Desktop** | **연결 구조 & 편집** | 넓은 화면에서 기억들의 관계를 정립하고 트리 구조를 재배치하는 'Curation' 경험. |

## 7. Decision Checklist (의사결정 기준)
새로운 기능이나 디자인을 제안할 때 아래 질문에 답할 수 있어야 합니다.
1. 이 기능이 팬의 **첫 순간**을 기록하는 데 도움이 되는가?
2. 이 디자인이 **따뜻한 스크랩북**의 느낌을 유지하고 있는가?
3. 이것이 데이터의 나열인가, 아니면 **감정의 흐름**인가?
4. IT 관리 툴(Admin)처럼 차갑게 느껴지지는 않는가?

## 8. Community Principle (커뮤니티 원칙)
Lovetree는 팬덤 커뮤니티의 **감정 공유 공간**으로서 가치를 합니다.

- **입덕 경로의 가이딩**: 완성된 LoveTree를 공유하여, 다른 팬들에게 "이렇게 입덕할 수 있다"는 길을 제시합니다.
- **Positive Energy Only**: 비판이나 부정보다 각자의 감정과 경험을 존중하는 분위기를 유지합니다.
- **돌봄의 연쇄**: 하나의 감정 기록이 다른 팬에게 감동이 되고, 그 감동이 또 다른 기록의 이유가 됩니다.

> Lovetree의 커뮤니티는 **"게시판"이 아니라 "러브트리 감상 공간"**입니다.

## 9. Design Guardrails
- **No Hard Edges**: 모든 컴포넌트에 부드러운 곡선을 적용하여 따뜻한 느낌을 유지합니다.
- **No Cold Palette**: 차가운 색상 대비를 지양하고, 감정을 보호하는 웜톤 톤을 기본으로 합니다.
- **No Mechanical Flow**: 노드 간의 연결은 자연스러운 감정의 흐름을 반영합니다.

> **구현 세부사항**은 [UI_DESIGN_SYSTEM.md](https://github.com/skerishKang/133-relovetree/blob/main/docs/design/UI_DESIGN_SYSTEM.md)를 참조하세요.

---

## Product Positioning Addendum

LoveTree/LoveBud는 정보를 정리하는 서비스가 아니라, 감정의 흐름을 기록하고 다시 감상하게 만드는 서비스다.

이 제품의 가장 작은 단위는 콘텐츠가 아니라 순간이다.  
순간은 정보 조각이 아니라 감정이 붙어 있는 기억의 입구이며, 모든 감상과 탐색은 이 입구에서 시작되어야 한다.

트리는 데이터 묶음이 아니라 감정 경로다.  
따라서 모든 화면은 “무엇이 몇 개 있는가”보다 “무엇이 먼저 느껴져야 하는가”를 먼저 결정해야 한다.

공개 영역은 피드, 게시판, 검색 결과 화면처럼 보이면 안 된다.  
LoveTree의 browse 경험은 누군가의 첫 순간과 이어진 감정을 조용히 따라가는 감상 허브여야 한다.

편집 경험도 일반 생산성 도구의 어조를 그대로 가져오면 안 된다.  
editor는 입력 폼을 채우는 곳이 아니라, 첫 순간을 심고 다음 순간을 이어 붙이는 공간이어야 한다.

빈 상태와 기본 상태는 “아직 없음”을 관리적으로 고지하는 자리가 아니다.  
첫 순간을 남기고 감정의 흐름을 시작하도록 자연스럽게 초대하는 장면이어야 한다.

UI의 핵심 기준은 기능 노출량이 아니라 감정 위계의 정확성이다.  
대표 순간, 첫 순간, 이어진 감정, 현재 트리 같은 핵심 요소는 언제나 수치, 상태, 관리 버튼보다 먼저 보여야 한다.

LoveTree/LoveBud의 모든 화면은 아래 원칙을 공유한다.

1. 먼저 느껴야 하는 감정을 정한 뒤 레이아웃을 설계한다.
2. 가장 먼저 보여야 하는 순간 또는 감정 단위를 화면의 1순위에 둔다.
3. 수치, 상태, 설정, 관리 액션은 감상 흐름을 해치지 않는 수준으로만 노출한다.
4. placeholder와 빈 상태 문구도 제품 톤 안에서 작성한다.
5. 게시판, 관리툴, 대시보드처럼 보이는 표현은 기본값으로 두지 않는다.

---

## Source of Truth Rule
제품의 방향성이 모호해질 때, 본 문서는 Lovetree 프로젝트의 **최상위 판단 기준**이 됩니다. `README.md`나 다른 운영 문서보다 본 문서의 철학이 우선합니다.
