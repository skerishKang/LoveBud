...
-- All failures use GENERIC_SOCIAL_A_POSTCONDITION_FAILED (bounded contract).
...
BEGIN
  ...
  -- Exactly four intended CHECK names, each unique catalog-wide
  SELECT count(*)::int INTO n FROM pg_constraint WHERE conname = 'social_idempotency_generic_target_pair_check';
  IF n <> 1 THEN RAISE EXCEPTION 'GENERIC_SOCIAL_A_POSTCONDITION_FAILED'; END IF;
  ...
  -- Semantic anchors on pair/kind checks exact (relation + contype + validated + hash)
  ...
  -- ── Functions exact (bounded postcondition category only) ───────────────
  expected_func := to_regprocedure('public.sync_social_idempotency_generic_target_from_legacy_memory()');
  IF expected_func IS NULL THEN RAISE EXCEPTION 'GENERIC_SOCIAL_A_POSTCONDITION_FAILED'; END IF;
  ...
  -- Triggers: exactly the two intended, correct shape, exact zero-arg function OID
