...
BEGIN
  ...
  -- Classify table state from nullability
  -- STATE_A: legacy NOT NULL + generic nullable
  -- STATE_B: legacy nullable + generic NOT NULL
  ...
  -- ── Migration A CHECK quartet always required (exact hash) ──────────────
  ...
  IF idem_state = 'A' THEN
    ...
    IF has_b_checks <> 0 THEN
      RAISE EXCEPTION 'GENERIC_SOCIAL_B_MIXED_STATE_REJECTED';
    END IF;

    -- Data: complete memory pairs only, legacy non-null, target_id = legacy
    ...
    -- Migration A function bodies: exact zero-arg overload + exact hash
    ...
    IF n <> 1 THEN
      RAISE EXCEPTION 'GENERIC_SOCIAL_B_MIGRATION_A_FUNCTION_DEFINITION_MISMATCH';
    END IF;
    SELECT pg_get_function_identity_arguments(p.oid), pg_get_function_result(p.oid), l.lanname, p.prosecdef,
    ...
    IF NOT FOUND
       OR f_ret IS DISTINCT FROM 'trigger'
       OR f_lang IS DISTINCT FROM 'plpgsql'
       OR f_sec
       OR f_vol IS DISTINCT FROM 'v'
  ...
  ELSIF idem_state = 'B' THEN
    ...
    -- STATE_B data: complete pair, memory|tree only, memory legacy match when present, tree legacy null
    SELECT count(*)::int INTO n FROM social_idempotency
    WHERE target_kind IS NULL OR target_id IS NULL
  ...
  -- ── Triggers (same intended shape both states; function OID matches current body) ─
  ...
  SELECT t.tgisinternal, t.tgtype, t.tgenabled, pn.nspname, p.proname,
         pg_get_function_identity_arguments(p.oid), pg_get_triggerdef(t.oid,false), t.tgfoid
