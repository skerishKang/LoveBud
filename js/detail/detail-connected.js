          ...
          if (flowMoments.length > 0) {
                connectedFragments.innerHTML = flowMoments.map(moment => {
                    ...
                    const thumbnailMarkup = moment.thumbnail
                        ? `<img src="${escapeHtml(moment.thumbnail)}" alt="${escapeHtml(moment.title || '')}" data-detail-thumbnail-fallback="youtube" style="width: 76px; height: 76px; border-radius: 1rem; object-fit: cover;">`
