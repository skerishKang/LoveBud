...
(function() {
    ...
    function renderEmotionTags(tags) {
        ...
        return tags.slice(0, 3).map(tag =>
            `<span class="emotion-tag" style="background:var(--primary-container);color:var(--on-primary-container);font-weight:700;font-size:12px;padding:6px 12px;">#${tag}</span>`
        ).join('');
    }
    ...
    function renderTreeCard(tree, index) {
        ...
        const cardId = `tree-card-${tree.id}`;
        
        let html = `
            <div class="tree-card" id="${cardId}" data-tree-id="${tree.id}" style="animation-delay: ${index * 0.05}s;">
                <!-- 트리 헤더: 제목 + 단계 아이콘 -->
    ...
    console.log('[LoveBudSearchCardRenderer] Search card renderer loaded v20260418-1');
})();