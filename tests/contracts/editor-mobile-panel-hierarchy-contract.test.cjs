...
test('mobile panel controller enforces mutual exclusivity and mobile-only lifecycle', () => {
  ...
  assert.match(script, /closePanel\(state\.activeKey, \{ restoreFocus: false \}\)/, 'opening one panel must close the other first');
...
test('mobile panel controller preserves focus and closing paths', () => {
  ...
  assert.match(script, /state\.returnFocusEl = document\.activeElement/, 'controller must store the previously focused element');
  assert.match(script, /focusTarget = state\.returnFocusEl \|\| record\.toggle/, 'controller must restore focus to toggle path');
