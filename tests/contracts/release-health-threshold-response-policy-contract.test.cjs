...
test('this contract is classified SOURCE_STATIC and registry/count literals are reconciled', () => {
  ...
  assert.match(registryContract, /expected post-child counts 790 \/ 578 \/ 191 \/ 21/);
  ...
  assert.ok(cfSmoke.includes('default_total,\\s*790'), 'CF smoke contract must embed default_total 790 literal');
