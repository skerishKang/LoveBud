...
test('7. expected post-child counts 774 / 570 / 188 / 16', () => {
  const data = buildReportData();
  assert.equal(data.default_total, 774);
  assert.equal(data.layer_counts.SOURCE_STATIC, 570);
  assert.equal(data.layer_counts.EXECUTED_FAKE, 188);
  assert.equal(data.layer_counts.EXECUTED_REAL_LOCAL, 16);
});
...
test('11. browser/process exact membership from reporter output', () => {
  ...
  assert.equal(browser.count + process.count, 16, 'browser + process = 16');
  const inv = readJson(CLASSIFICATION_PATH);
  const realLocal = inv.entries.filter(e => e.layer === 'EXECUTED_REAL_LOCAL');
  assert.equal(realLocal.length, 16);
