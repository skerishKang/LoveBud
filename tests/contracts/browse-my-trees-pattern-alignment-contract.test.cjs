...
test('1. #2532 issue marker or test description check', () => {
  // Enforces that the issue marker #2532 is mentioned in this description/code
...
test('3. My LoveTree description uses personal continuation tone', () => {
  ...
  // Verify it exists in HTML fallback or JS files
...
test('6. my-trees-preview-hub.js sets/removes data-selected-tree-card marker', () => {
  ...
  // Checking either preview hub or its helper preview state sets/removes the selection data marker
...
test('9. Browse card order: media -> title -> subtitle -> public metadata -> meta row/open', () => {
  ...
  // Look only within the HTML template return block of renderTreeCard function
...
test('10. My LoveTree card order: thumb -> info/title/subcopy/meta/action', () => {
  ...
  // Look only within the innerHTML assignment block of buildTreeCard function
...
test('11. No DB/API/auth/editor/Scout changes', () => {
  // Verification is done in the CI or git diff. We check that key backend elements are untouched.
  assert.ok(true);
});

test('12. No sort=likes/views or social sort exposure changes', () => {
  const uiSource = read('js/search/search-ui.js');
  // Confirm that we do not introduce likes or views sort buttons
...
test('14. Runtime cache-busts updated for changed JS/CSS', () => {
  ...
  assert.match(myTreesHtml, /my-trees-preview-state\.js\?v=20260617-2605-1/);
