...
test('1. My Trees bundle imports mobile controls balance override last', () => {
  ...
  const balanceIdx = css.indexOf('my-trees-mobile-controls-balance.css?v=20260618-2680-1');
...
test('2. Mobile sort control uses a balanced partial width', () => {
  const css = read('css/my-trees/my-trees-mobile-controls-balance.css');
  assert.match(css, /@media\s*\(max-width:\s*768px\)/);
  assert.match(css, /\.my-trees-results-controls\s+\.sort-control\s*{[^}]*flex:\s*0\s+1\s+68%;[^}]*max-width:\s*260px;[^}]*}/s);
});

test('3. View mode keeps trailing separation from the shorter sort control', () => {
  const css = read('css/my-trees/my-trees-mobile-controls-balance.css');
  assert.match(css, /\.my-trees-results-controls\s*{[^}]*gap:\s*14px;[^}]*}/s);
