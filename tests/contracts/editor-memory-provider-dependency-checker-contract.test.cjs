...
test('memory provider dependency delegation preserves provider construction paths', () => {
  ...
  assert.match(editorSource, /editorTreeHelpers,\s*getTreeMemories:\s*\(\)\s*=>\s*treeMemories\(\),\s*findRootMemory:\s*deps\.findRootMemory,\s*canonicalRootId,\s*treeId,\s*i18n/s);
