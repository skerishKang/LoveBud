...
function fakeBootstrapQueryResult(queryObject) {
  ...
  if (name === 'lovebud-migration-lock-acquire-v1') return { rows: [{ acquired: true }] };
  if (name === 'lovebud-migration-lock-check-v1') return { rows: [{ held: true }] };
  if (name === 'lovebud-migration-lock-release-v1') return { rows: [{ released: true }] };
  if (text === 'BEGIN') return { rows: [] };
  if (/INSERT INTO schema_migration_ledger/i.test(text)) {
    return { rows: [{ migration_id: values[0], content_checksum: values[1] }] };
  }
...
function makeFakeSession(tracking) {
  ...
  return {
    query: async function (queryObject) {
      ...
      return fakeBootstrapQueryResult(queryObject);
    },
