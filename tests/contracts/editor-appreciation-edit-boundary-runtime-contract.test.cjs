...
test('mounted view/edit templates have unique IDs and no appreciation action duplication', () => {
  // #3563: view markup is generated by the shared builder under owner authority.
  const ctx = { window: {}, globalThis: null };
  ctx.globalThis = ctx;
  vm.createContext(ctx);
  vm.runInContext(read('js/shared/canonical-appreciation-detail-presentation.js'), ctx);
  const viewHtml = ctx.window.LoveBudCanonicalAppreciationDetailPresentation.buildDetailViewModeHtml({
    authority: 'owner'
  });
