...
function createContext() {
  ...
  const window = {
    ...
    loveBudCache: {},
    LoveBudCache: {
      get(key) { return memoryCache.has(key) ? memoryCache.get(key) : null; },
      set(key, value) { memoryCache.set(key, value); },
      clear(key) { memoryCache.delete(key); sessionStorage.removeItem('lb_' + key); },
    },
    location: { origin: 'https://lovebud.test', search: '', pathname: '/pages/my-trees' },
    requestIdleCallback(callback) { callback(); },
  };
  const context = vm.createContext({
    window, localStorage, sessionStorage, document, console, URL, URLSearchParams,
...
function privateKeys(storage) {
  ...
  for (let i = 0; i < storage.length; i += 1) {
    ...
    if (key === 'lovebud_my_trees_list_cache' || key === 'lovebud_trees_cache' ||
        (key && key.startsWith('tree_detail_')) || (key && key.startsWith('tree_memories_'))) {
      out.push(key);
    }
  }
  return out.sort();
...
function titlesOf(trees) { return Array.from(trees || [], (tree) => tree.title); }
...
test('#3928 bounded purge preserves same-UID reuse and unrelated/public storage', () => {
  ...
  assert.deepEqual(titlesOf(cache.readPrivateCacheRecord('lovebud_my_trees_list_cache', 'user-a').data), ['A private tree']);
  ...
  assert.equal(cache.writePrivateCacheRecord('tree_detail_b-tree', 'user-b', { data: { id: 'b-tree' } }, authorityB), true);
...
test('#3928 legacy or mismatched private records are misses and removed', () => {
  ...
  env.localStorage.setItem('lovebud_my_trees_list_cache', JSON.stringify({
    data: [{ id: 'legacy-a', title: 'Legacy A title' }], expiry: Date.now() + 60000,
  }));
  assert.equal(cache.readPrivateCacheRecord('lovebud_my_trees_list_cache', 'user-b'), null, 'legacy record without UID is never returned');
  assert.equal(env.localStorage.getItem('lovebud_my_trees_list_cache'), null, 'legacy record is removed');
  env.localStorage.setItem('tree_detail_wrong', JSON.stringify({ uid: 'user-a', data: { id: 'wrong' } }));
...
test('#3928 canonical preload cannot repopulate User A data after UID transition', async () => {
  ...
  env.window.LoveBudAuthSession.preloadRedirectTargetData({
    ...
    apiClient: { getTrees: () => treesDeferred.promise },
    logger: { log() {}, warn() {} },
  });
  cache.syncConfirmedPrivateOwner('user-b');
...
test('#3928 My Trees cache-first paint is same-UID only and late A list results are stale', async () => {
  ...
  cache.writePrivateCacheRecord('lovebud_my_trees_list_cache', 'user-a', {
    data: [{ id: 'a-cached', title: 'A cached title' }], expiry: Date.now() + 60000, cachedAt: Date.now(),
  }, authorityA);
  ...
  const firstLoad = env.window.LoveBudMyTreesData.loadTrees({
    setState() {}, stateEnum: { LOADING: 'loading', ERROR: 'error' },
    renderTrees(trees) { renders.push(titlesOf(trees)); }, acknowledgeUi() { return true; }, showToast() {},
  });
  ...
  const secondLoad = env.window.LoveBudMyTreesData.loadTrees({
    setState() {}, stateEnum: { LOADING: 'loading', ERROR: 'error' },
    renderTrees(trees) { secondRenders.push(titlesOf(trees)); }, acknowledgeUi() { return true; }, showToast() {},
  });
