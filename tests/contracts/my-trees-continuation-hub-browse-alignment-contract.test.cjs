...
test('My Trees hub keeps its non-media focus surface and shared visual rhythm', () => {
  ...
  assert.match(actions, /\\.my-trees-hub-share-btn[\s\S]*?min-height:\s*42px;[\s\S]*?border-radius:\s*999px;/, '감상 링크 복사 tertiary action rhythm');
  assert.match(actions, /\\.my-trees-hub-visibility-btn[\s\S]*?min-height:\s*42px;[\s\S]*?border-radius:\s*999px;/, '공개 범위 quaternary action rhythm');
...
test('My Trees hub summary uses Browse-parity typography', () => {
  ...
  assert.match(
    content,
    /\.my-trees-hub-summary\s*\{\s*font-size:\s*14px;[^}]*padding:\s*0;[^}]*line-height:\s*1\.6;[^}]*\}/,
    '.my-trees-hub-summary padding must be 0 matching Browse'
  );
...
test('My Trees flow controls line-height matches Browse parity', () => {
    ...
    // .my-trees-hub-flow-controls must have line-height matching Browse controls line-height
    assert.match(
        flow,
        /\.my-trees-hub-flow-controls\s*\{\s*margin-top:\s*9px;[^}]*line-height:\s*1\.[12];[^}]*\}/,
        '.my-trees-hub-flow-controls must have line-height matching Browse controls line-height'
    );
