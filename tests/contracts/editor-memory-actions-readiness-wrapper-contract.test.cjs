...
test('editor keeps memory actions creation and assignment intact', () => {
  ...
  assert.match(editorSource, /const \{\s*enterEditMode,\s*exitEditMode,\s*saveMemoryEdit,\s*deleteMemory(?:\s*,\s*disconnectMemory)?(?:\s*,\s*connectMemory)?(?:\s*,\s*validateConnectCandidate)?\s*\}\s*=\s*memoryActions/);
});
