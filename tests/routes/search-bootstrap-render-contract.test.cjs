...
// ---------------------------------------------------------------------------
// Contract: no API / Firebase / auth / Viewer / Editor changes in this diff
// ---------------------------------------------------------------------------

test('changed files do not include API, Firebase, auth, Viewer, or Editor modules', () => {
  var allowedPrefixes = [
    'js/search/index.js',
    'pages/search.html',
    'tests/routes/search-bootstrap-render-contract.test.cjs',
    'tests/contracts/browse-preview-share-mvp-contract.test.cjs',
    'tests/contracts/browse-view-count-alias-precedence-contract.test.cjs',
    'tests/contracts/dom-xss-renderer-guardrail-contract.test.cjs',
    'tests/contracts/search-preview-comments-panel-xss-contract.test.cjs',
    'tests/contracts/browse-card-viewcount-display-contract.test.cjs',
    'tests/routes/search-ui-remaining-fallback-inventory.test.cjs',
    'tests/routes/search-ui-remaining-fallback-inventory.test.js',
    'tests/routes/search-runtime-modules.test.cjs',
    'tests/routes/search-script-order-contract.test.cjs',
  ];

  var child = require('child_process');
  var output;
  try {
    output = child.execSync('git diff --name-only origin/main...HEAD', { cwd: ROOT, timeout: 5000, encoding: 'utf8' });
  } catch (e) {
    // origin/main not in refspec — skip
    return;
  }
  var changedFiles = output.trim().split('\n').filter(Boolean);
  for (var i = 0; i < changedFiles.length; i++) {
    var file = changedFiles[i];
    var allowed = false;
    for (var j = 0; j < allowedPrefixes.length; j++) {
      if (file === allowedPrefixes[j] || file.indexOf(allowedPrefixes[j]) === 0) {
        allowed = true;
        break;
      }
    }
    assert.ok(allowed, 'Changed file "' + file + '" is not in the allowed list');
  }
});
